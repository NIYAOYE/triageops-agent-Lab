...
- Tavily 网页搜索、受限文件读取、受限 Python 执行和 SupportOps 附件分析工具
...
- `src/tool_use_agent/tools/`：搜索、文件读取、Python 执行、日志扫描、JSON 查询、CSV 概览和工具注册
...
运行时工具通过 `ToolRegistry` 暴露给 Qwen Function Calling，并统一返回 `ToolResult`。

| Tool | Purpose |
| --- | --- |
| `web_search` | 使用 Tavily 获取带 URL 的外部搜索结果 |
| `read_file` | 读取工作区内受控文本文件 |
| `python_exec` | 在隔离临时目录中执行受限 Python 分析代码 |
| `log_scan` | 按关键词或正则扫描工作区日志，并返回行号与上下文 |
| `json_query` | 对工作区 JSON 文件执行简单点路径查询，例如 `errors[0].code` |
| `csv_profile` | 汇总 CSV 行数、列名、空值、重复值、样本行和可选分组计数 |

`log_scan`、`json_query` 和 `csv_profile` 只读取配置工作区内的文本文件，复用路径越界、编码、
二进制内容和大小限制边界，适合在工单调查中快速分析附件。
...
- `read_file`、`log_scan`、`json_query` 和 `csv_profile` 拒绝绝对路径、路径穿越、符号链接越界、二进制文件和超大文件。
